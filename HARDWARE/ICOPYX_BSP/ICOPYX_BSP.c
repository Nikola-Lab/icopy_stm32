...
u16 MAINBATCHECKTASK(u8 what)
{
	...
	if (what == 0)
	{
		...
		else if(step == 0 && inprocess == 1 && g_Tim2Array[eTimbat] < 10000) //充电期间，要考虑是否真的在充电
		{
			if (MAINCHARGETASK(1) == 0  || otgon == 1)
			{
				...
				inprocess = 0;
				updateok = 0;
				//printf("OTG ON,DO NOT WAIT FOR CHARGE");
				//fflush(stdout);
			}
		}
	...
	if(batvolsense <= BATWITHLOADTHR && step == 0 && inprocess == 0)
	{
		...
		if ((BATvolavl <= BATWITHLOADTHR) && (!MAINCHARGETASK(1)))
		{
	...
	if (aimvol != 0 && outvol != 0)
	{
		...
		else
		{
			if (g_Tim2Array[eTimbatmv] > timetowait)
			{
				...
				//printf("outpst %d aimpst %d\r\n", BATVOL2PERCENT(outvol), BATVOL2PERCENT(aimvol));
				//fflush(stdout);
			}
		}
